...
my_tsne_tibble <- my_tsne_tibble %>% 
  left_join(tidy_conditions, by=c("conditions"="condition"))
...
```{r}
transposed_tsne_out_df<-as_data_frame(transposed_tsne_out$Y)
transposed_tsne_out_g_df<-bind_cols(SC_expression_clean %>% select(gene), transposed_tsne_out_df)
names(transposed_tsne_out_g_df)<-c("gene","tSNE1", "tSNE2")
transposed_tsne_out_g_df
```
...
tsne_coloring <- function(filter_query) {
  ...
  t_filter_df <- transposed_tsne_out_g_df %>% 
    mutate({{ filter_query }} := map_lgl(gene, function(x) x %in% filtered_values))
  
  ggplot(t_filter_df, aes_string("tSNE1", "tSNE2", color = filter_query)) + 
    geom_point()
  }

t_g <- tsne_coloring("protein") 
  ggsave(here("output/tsne_nottransposed.png"), t_g, width = 16, height = 9)
```
