...
go_terms <- str_replace_all(go_terms, " ", "_")
...
shinyServer(function(input, output, session) {
    ...
    #strain_meta_grouping <- read_csv("data/strain_meta_grouping.csv")
    group_table <- read_csv("data/grouping_table.csv")
    ...
    output$umap <- renderPlot({
        ...
        filtered_values <- go_annotation2[grep(filter_query, go_annotation2[,3]),1]

        column_name <- str_replace_all(filter_query, " ", "_")
